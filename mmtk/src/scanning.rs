...
impl Scanning<OpenJDK> for VMScanning {
    ...
    fn thread_stack_depth(mutator: &'static Mutator<OpenJDK>) -> usize {
        let tls = mutator.get_tls();
        unsafe { ((*UPCALLS).thread_stack_depth)(tls) }
    }

    fn thread_stack_size(mutator: &'static Mutator<OpenJDK>) -> usize {
        let tls = mutator.get_tls();
