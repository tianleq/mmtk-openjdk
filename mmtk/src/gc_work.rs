...
use crate::scanning::create_process_edges_work_non_stack_roots;
use mmtk::scheduler::*;
use mmtk::MMTK;
...
impl<E: ProcessEdgesWork<VM = OpenJDK>> GCWork<OpenJDK> for ScanUniverseRoots<E> {
    fn do_work(&mut self, _worker: &mut GCWorker<OpenJDK>, _mmtk: &'static MMTK<OpenJDK>) {
        unsafe {
            ((*UPCALLS).scan_universe_roots)(create_process_edges_work_non_stack_roots::<E> as _);
        }
    }
}
...
impl<E: ProcessEdgesWork<VM = OpenJDK>> GCWork<OpenJDK> for ScanJNIHandlesRoots<E> {
    fn do_work(&mut self, _worker: &mut GCWorker<OpenJDK>, _mmtk: &'static MMTK<OpenJDK>) {
        unsafe {
            ((*UPCALLS).scan_jni_handle_roots)(create_process_edges_work_non_stack_roots::<E> as _);
        }
    }
}
...
impl<E: ProcessEdgesWork<VM = OpenJDK>> GCWork<OpenJDK> for ScanObjectSynchronizerRoots<E> {
    fn do_work(&mut self, _worker: &mut GCWorker<OpenJDK>, _mmtk: &'static MMTK<OpenJDK>) {
        unsafe {
            ((*UPCALLS).scan_object_synchronizer_roots)(
                create_process_edges_work_non_stack_roots::<E> as _,
            );
        }
    }
}
...
impl<E: ProcessEdgesWork<VM = OpenJDK>> GCWork<OpenJDK> for ScanManagementRoots<E> {
    fn do_work(&mut self, _worker: &mut GCWorker<OpenJDK>, _mmtk: &'static MMTK<OpenJDK>) {
        unsafe {
            ((*UPCALLS).scan_management_roots)(create_process_edges_work_non_stack_roots::<E> as _);
        }
    }
}
...
impl<E: ProcessEdgesWork<VM = OpenJDK>> GCWork<OpenJDK> for ScanJvmtiExportRoots<E> {
    fn do_work(&mut self, _worker: &mut GCWorker<OpenJDK>, _mmtk: &'static MMTK<OpenJDK>) {
        unsafe {
            ((*UPCALLS).scan_jvmti_export_roots)(
                create_process_edges_work_non_stack_roots::<E> as _,
            );
        }
    }
...
impl<E: ProcessEdgesWork<VM = OpenJDK>> GCWork<OpenJDK> for ScanAOTLoaderRoots<E> {
    fn do_work(&mut self, _worker: &mut GCWorker<OpenJDK>, _mmtk: &'static MMTK<OpenJDK>) {
        unsafe {
            ((*UPCALLS).scan_aot_loader_roots)(create_process_edges_work_non_stack_roots::<E> as _);
        }
    }
}
...
impl<E: ProcessEdgesWork<VM = OpenJDK>> GCWork<OpenJDK> for ScanSystemDictionaryRoots<E> {
    fn do_work(&mut self, _worker: &mut GCWorker<OpenJDK>, _mmtk: &'static MMTK<OpenJDK>) {
        unsafe {
            ((*UPCALLS).scan_system_dictionary_roots)(
                create_process_edges_work_non_stack_roots::<E> as _,
            );
        }
    }
...
impl<E: ProcessEdgesWork<VM = OpenJDK>> GCWork<OpenJDK> for ScanCodeCacheRoots<E> {
    fn do_work(&mut self, _worker: &mut GCWorker<OpenJDK>, _mmtk: &'static MMTK<OpenJDK>) {
        unsafe {
            ((*UPCALLS).scan_code_cache_roots)(create_process_edges_work_non_stack_roots::<E> as _);
        }
    }
}
...
impl<E: ProcessEdgesWork<VM = OpenJDK>> GCWork<OpenJDK> for ScanVMThreadRoots<E> {
    fn do_work(&mut self, _worker: &mut GCWorker<OpenJDK>, _mmtk: &'static MMTK<OpenJDK>) {
        unsafe {
            ((*UPCALLS).scan_vm_thread_roots)(create_process_edges_work_non_stack_roots::<E> as _);
        }
    }
}
