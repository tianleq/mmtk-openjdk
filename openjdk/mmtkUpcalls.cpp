...
#ifdef MMTK_ENABLE_THREAD_LOCAL_GC
    assert(!cur->mmtk_thread_local_gc_active(), "global gc and local gc has to be mutually exclusive");
...
static void mmtk_scan_universe_roots(SlotsClosure closure) { MMTkRootsClosure cl(closure, 1); MMTkHeap::heap()->scan_universe_roots(cl); }
static void mmtk_scan_jni_handle_roots(SlotsClosure closure) { MMTkRootsClosure cl(closure, 2); MMTkHeap::heap()->scan_jni_handle_roots(cl); }
static void mmtk_scan_object_synchronizer_roots(SlotsClosure closure) { MMTkRootsClosure cl(closure, 3); MMTkHeap::heap()->scan_object_synchronizer_roots(cl); }
static void mmtk_scan_management_roots(SlotsClosure closure) { MMTkRootsClosure cl(closure, 4); MMTkHeap::heap()->scan_management_roots(cl); }
static void mmtk_scan_jvmti_export_roots(SlotsClosure closure) { MMTkRootsClosure cl(closure, 5); MMTkHeap::heap()->scan_jvmti_export_roots(cl); }
static void mmtk_scan_aot_loader_roots(SlotsClosure closure) { MMTkRootsClosure cl(closure, 6); MMTkHeap::heap()->scan_aot_loader_roots(cl); }
static void mmtk_scan_system_dictionary_roots(SlotsClosure closure) { MMTkRootsClosure cl(closure, 7); MMTkHeap::heap()->scan_system_dictionary_roots(cl); }
static void mmtk_scan_code_cache_roots(SlotsClosure closure) { assert(false, "");  MMTkRootsClosure cl(closure, 8); MMTkHeap::heap()->scan_code_cache_roots(cl); }
static void mmtk_scan_string_table_roots(SlotsClosure closure) { MMTkRootsClosure cl(closure, 9); MMTkHeap::heap()->scan_string_table_roots(cl); }
static void mmtk_scan_class_loader_data_graph_roots(SlotsClosure closure) { MMTkRootsClosure cl(closure, 10); MMTkHeap::heap()->scan_class_loader_data_graph_roots(cl); }
static void mmtk_scan_weak_processor_roots(SlotsClosure closure) { MMTkRootsClosure cl(closure, 11); MMTkHeap::heap()->scan_weak_processor_roots(cl); }
static void mmtk_scan_vm_thread_roots(SlotsClosure closure) { MMTkRootsClosure cl(closure, 12); MMTkHeap::heap()->scan_vm_thread_roots(cl); }
...
static void mmtk_execute_thread_local_gc(void *tls)
{
  ...
  // therefore, it is considered as unsafe and no 
